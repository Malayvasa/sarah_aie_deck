...
export function TerminalWindow({
	...
	return (
		<div
			className={cn(
				"terminal-theme flex min-h-0 w-full flex-col overflow-hidden border font-mono text-xs leading-relaxed",
...
function TerminalHeaderProgress({
	...
	const ariaLabel = label != null && label !== "" ? String(label) : "Progress";
	...
	return (
		<div className="flex shrink-0 items-center gap-2 text-[13px]">
			...
			<span
				aria-label={
					showValue ? `${ariaLabel} ${progress.toFixed(2)}%` : ariaLabel
				}
				aria-valuemax={100}
				aria-valuemin={0}
				aria-valuenow={progress}
				className="terminal-panel-sm relative inline-flex h-[1em] w-16 items-stretch overflow-hidden"
				role="progressbar"
...
export function TerminalHeader({
	...
	return (
		...
		>
			...
			{path && (
				<div
					className="ml-1.5 min-w-0 flex-1 truncate text-[13px]"
